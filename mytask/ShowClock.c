...
extern TaskHandle_t xkey_taskHandle;
extern TaskHandle_t xGetQueneTaskHandle;
extern u8g2_t u8g2;
...
void ShowClockTimeTask(void *params)
{
   ...
   Keydata	key_data1;	
	while(1)
	{
		...
		if(clock_flag == 0)
		{
			pdPASS == xQueueReceive(g_xQueuekey, &key_data1, portMAX_DELAY);
		}
		/* seclect */
	...
	else if(key_data1.repeatdata==1)	
		{
			if(seclect_flag!=4)
			{
				g_clock_num[seclect_flag]++;
